...
//	public List<CartItem> getAllItemsByCustomer(String userName){
//		List<CartItem> allCartItems = cartRepository.findAll();
//		return findAllCartItemsCustomerId(allCartItems, userName);
//	}//End method

//	//loop through all cart items to find cart items by customer id
//	public List<CartItem> findAllCartItemsCustomerId(List<CartItem> allCartItems, String userName){
//		List<CartItem> allCartItemsByCustomer = new ArrayList<CartItem>();
//		
//		for(int i = 0; i < allCartItems.size(); i ++) {
//			
//			if(allCartItems.get(i).getUser().getUserName() == userName) {
